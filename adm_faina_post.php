...
<?php
    if( !isset($_SESSION['uaccess']) || $_SESSION['uaccess']<400 || !isset($_GET['post']) || !($_GET['post']=="new"||is_numeric($_GET['post']))){
    ...
    if( $_POST && isset($_SESSION['ulogin']) ){
        if(isset($_POST['add'])){
            ...
            if ($stmt = $conn->prepare("INSERT INTO f_posts (image_id, state, title, content, user_entry, pub_by, pub_date) VALUES (?, ?, ?, ?, ?, ?, CURRENT_DATE)")) {
        ...
        $conn = new mysqli($servername, $username, $password);
        $conn->select_db("aauav-nei");
        if ($stmt = $conn->prepare("UPDATE f_posts SET image_id=?, state=?, title=?, content=?, user_entry=?, chg_by=?, chg_date=CURRENT_DATE WHERE id=?")) {
            $stmt->bind_param("sssssss",  $_POST['image'],$_POST['state'], $_POST['title'], $_POST['content'], $_POST['user_entry'], $_SESSION['ulogin'], $_GET['post']);
            $stmt->execute();
        }
    }
        header('Location: adm_faina.php');
        exit();
    }
    ...
    if($_GET['post']!="new"){
        ...
        if ($stmt = $conn->prepare("SELECT title, state, content, user_entry, image_id FROM f_posts WHERE f_posts.id=?")) {
    ...
    $images = null;
    $conn = new mysqli($servername, $username, $password); $conn->select_db("aauav-nei");
    if ($stmt2 = $conn->prepare("SELECT id, name FROM images")) {
        $stmt2->execute();
