...
<?php
    if( !isset($_SESSION['uaccess']) || $_SESSION['uaccess']<498){
    ...
    if( $_POST && isset($_SESSION['ulogin']) && isset($_POST['post_id']) ){
        if(isset($_POST['toggle_state'])){
            ...
            if ($stmt = $conn->prepare("UPDATE n_posts SET state=CASE WHEN state=1 THEN 0 ELSE 1 END, chg_by=?, chg_date=CURRENT_DATE WHERE id=?")) {
                ...
                <table id="posts" class="display responsive nowrap table table-sm table-striped" cellspacing="0" width="100%">
                    <thead>
                        <tr>
                            <td colspan="4"></td>
                            <td>
                                <a href="adm_posts_post.php?post=new" class="btn btn-sm btn-success ml-3">
                                    Nova publicação
                                </a>
                            </td>
                        </tr>
                    ...
                    <?php
                        $conn = new mysqli($servername, $username, $password);
                        $conn->select_db("aauav-nei");
                        if ($stmt = $conn->prepare("SELECT id, title, state, concat(pub_date, ' (por ', pb_users.u_name, ')') as publication, (case when chg_by IS NULL THEN '' ELSE concat(chg_date, ' (por ', chg_users.u_name, ')') END) as last_change FROM n_posts LEFT JOIN users AS chg_users ON chg_by = chg_users.u_login LEFT JOIN users AS pb_users ON pub_by = pb_users.u_login ORDER BY pub_date DESC")) {
                            $stmt->execute();
                            ...
                            while($row = $result->fetch_assoc()) { ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="posts_post.php?post=<?php echo $row['id']; ?>" target="_blank">
                                    ...
                                    <td align="center">
                                        <form class="form-inline" method="POST" style="max-width: 15em;">
                                            ...
                                            <a href="adm_posts_post.php?post=<?php echo $row['id']; ?>" class="btn btn-sm btn-success ml-3">
                                                Editar
                                            </a>
                                        </form>
                                    </td>
