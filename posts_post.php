...
<?php
    ...
    if( $_POST && isset($_SESSION['ulogin']) && isset($_POST['guests']) ){
        if(isset($_POST['confirm'])){
            ...
            if ($stmt = $conn->prepare("INSERT INTO inscricoes_nei (post_id, user_login, guests) SELECT id, ?, ? FROM n_posts WHERE user_entry='1' AND id=?")) {
    ...
    $_SESSION['iddopagamento'] = $_GET['post'];
    
    $conn = new mysqli($servername, $username, $password);
    $conn->select_db("aauav-nei");

    $post = null;
    if ($stmt = $conn->prepare("SELECT title, content, name, path, user_entry FROM n_posts JOIN images_nei ON images_nei.id = image_id WHERE state=1 AND n_posts.id =?")) {
    ...
    if( $post_has_user_entry!="0" && $user_login ) {
        if ($stmt2 = $conn->prepare("SELECT estado, guests FROM inscricoes_nei WHERE post_id=? AND user_login=?")) {
    ...
    if( $user_login ) {
        if ($stmt3 = $conn->prepare("SELECT u_name, guests, estado FROM inscricoes_nei JOIN users ON user_login = u_login WHERE post_id = ?")) {
    ...
    $_SESSION['titulodopagamento'] = utf8_encode($post['title']);
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="card mb-5" style="width: 100%;">
                    <img class="card-img-top" src="<?php echo implode("/",array_slice(explode('/', $_SERVER['PHP_SELF']), 0, -1)); ?>/upload/NEI/<?php echo utf8_encode($post['path']); ?>" style="width: 100%;" title="<?php echo utf8_encode($post['name']); ?>">
                    <div class="card-body">
                    ...
                    <div class="card-footer text-center">
                        ...
                        <?php while( !is_null($registered_users) && $row = $registered_users->fetch_assoc()) { ?>
                            <tr>
                                ...
                                <td><?php if( $row['estado'] == '0'){ echo 'Aguarda Pagamento'; }
                                if( $row['estado'] == '1'){ echo 'Pago'; }?></td>
                            </tr>
                            <?php $incritos += 1+$row['guests']; ?>
                            <?php if( $row['estado'] != '0' ) {$confirmados += 1+$row['guests']; } ?>
                        <?php } ?>
                            <tr>
                                <th colspan="3" style="text-align: right">Pagamentos: <?php echo $confirmados; ?> / <?php echo $incritos; ?></th>
                            </tr>
                        </table>
                    </div>
                    <?php } ?>
                    ...
                    <div class="card-footer text-center">
                        ...
                        <?php }?>
                        <?php if(isset($_SESSION['uaccess']) && $_SESSION['uaccess']>=499) : ?>
                                    <a href="pagamentos.php" class="btn btn-sm btn-success ml-3">
                                        Pagar
                                    </a>
                                <?php endif; ?>
