...
<?php startblock('body') ?>
	<?php
		...
		if ($_POST && isset($_POST['chng_passwd']) && !empty($_POST['curr_password']) 
			...
			if ($stmt = $conn->prepare("SELECT u_pwd FROM users WHERE u_login=?")) {
				...
				if ($result->num_rows > 0) {
					...
					if($wp_hasher->CheckPassword($_POST['curr_password'], $row["u_pwd"])) {
						if ($stmt2 = $conn->prepare("UPDATE users SET u_pwd=? WHERE u_login=?")) {
